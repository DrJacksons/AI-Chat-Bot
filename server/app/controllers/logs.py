...
from server.app.repositories import LogsRepository
from server.core.controller import BaseController
from server.app.schemas.responses.logs import LogsResponseModel, LogResponseModel
...
class LogsController(BaseController[Logs]):
    ...
    async def get_logs(self, user_id: str, skip: int = 0, limit: int = 10):
        ...
        tokens_count = await self.logs_repository.get_tokens_count(user_id=user_id)
        ...
        return LogsResponseModel(message="Logs returned successfully", logs_count=logs_count, tokens_count=tokens_count, logs=logs)
