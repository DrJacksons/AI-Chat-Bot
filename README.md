...
使用 alembic 进行数据库迁移：
```shell
# 初始化alembic模板配置（生成 alembic.ini 和 migrations/ 目录）
alembic init migrations
# 修改 alembic.ini 中的 sqlalchemy.url 为实际数据库连接字符串
sqlalchemy.url = postgresql+psycopg2://root:123456@127.0.0.1:5432/test
...
# 更新表结构
alembic revision --autogenerate -m "create tables from models"
# 数据库迁移
