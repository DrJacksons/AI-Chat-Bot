...
class Column(BaseModel):
    ...
    def is_expression_valid(cls, expr: str) -> Optional[str]:
        ...
        try:
            ...
            raise ValueError(f"无效的SQL表达式: {expr}. Error: {str(e)}")
...
class Destination(BaseModel):
    ...
    format: str = Field(..., description="输出文件的格式")
    path: str = Field(..., description="输出文件的路径")

    @field_validator("format")
    @classmethod
    def is_format_supported(cls, format: str) -> str:
        if format not in LOCAL_SOURCE_TYPES:
            raise ValueError(f"不支持的目标格式: {format}")
        return format
...
class SQLConnectionConfig(BaseModel):
    """
    Oracle、MySQL和PostgreSQL的常用连接配置。
    """
...
class Source(BaseModel):
    ...
    def is_compatible_source(self, source2: "Source"):
        """
        检查两个源是否兼容以在View中进行组合。

        两个source被认为是兼容的条件:
        - 都是local sources。
        - 都是remote sources，且连接配置相同。

        兼容的source可以在同一个View中一起使用.
    ...
    def validate_type_and_fields(cls, values):
        ...
        if _type in LOCAL_SOURCE_TYPES:
            ...
            raise ValueError(f"不支持的source类型: {_type}")
...
class SemanticLayerSchema(BaseModel):
    ...
    destination: Optional[Destination] = Field(None, description="数据集的存储位置")
    update_frequency: Optional[str] = Field(None, description="数据集的更新频率")
    group_by: Optional[List[str]] = Field(None, description="要分组的columns列表。每个非聚合column必须包含在group_by中。")
